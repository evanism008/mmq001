...
<template>
	<div style="background:#ffffff;height:600px">
		...
		>
			...
			<el-table-column prop="goods_stype_name" label="类型" width="100" align="center"> </el-table-column>
			<!-- hotType为1时是热门 -->
		...
		<el-dialog title="添加商品信息" :visible.sync="dialogFormVisiblet" width="40%">
			<el-form :model="form" ref="form">
				...
				<el-form-item label="商品类型" :label-width="formLabelWidth">
					<el-select v-model="form.goods_stype_id" placeholder="请选择商品类型">
						<el-option label="华为手机" value="1"></el-option>
						<el-option label="小米手机" value="2"></el-option>
						<el-option label="Redmi Note" value="3"></el-option>
						<el-option label="vivo手机" value="4"></el-option>
						<el-option label="oppo手机" value="5"></el-option>
						<el-option label="一加手机" value="6"></el-option>
						<el-option label="苹果手机" value="7"></el-option>
						<el-option label="其他手机" value="8"></el-option>
						<el-option label="周边配件" value="9"></el-option>
					</el-select>
				</el-form-item>
		...
		methods: {
			...
			getDataList() {
				this.$http
					...
					.then(res => {
						this.data = res.data.filter(function(item) {
			...
			onSearch() {
				...
				this.$http
					...
					.then(res => {
						this.data = res.data.filter(item => item.goods_stype_id != 9);
						this.data.forEach((v, index) => {
							v.key = v.goods_stype_id;
							v.index = index;
						});
